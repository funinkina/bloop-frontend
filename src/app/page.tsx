...
export default function HomePage() {
  ...
  const [serverHealth, setServerHealth] = useState<{ status: 'ok' | 'busy' | 'error' | 'loading', message: string }>({ status: 'loading', message: 'Pinging server...' });
        ...
        <div className='absolute h-fit w-fit top-6 rounded-xl px-4 py-2 gap-1 left-1/2 transform -translate-x-1/2 flex flex-col justify-center items-center z-20 bg-black/90 text-amber-50 blueh-fit overflow-hidden focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 shadow hover:bg-black/90 whitespace-pre md:flex group transition-all duration-300 ease-out hover:ring-2 hover:ring-blue-950 hover:ring-offset-2'>
          ...
          {serverHealth && (
            <div className="flex items-center space-x-1 text-xs">
              <span
                className={`w-2 h-2 rounded-full ${serverHealth.status === 'ok' ? 'bg-green-400 animate-pulse' :
                  serverHealth.status === 'busy' ? 'bg-yellow-500 animate-pulse' :
                    serverHealth.status === 'loading' ? 'bg-gray-400 animate-ping' :
                      'bg-red-500'
                  }`}
                title={`Status: ${serverHealth.status}`}
              ></span>
              <span className={
                `font-medium ${serverHealth.status === 'ok' ? 'text-green-400' :
                  serverHealth.status === 'busy' ? 'text-yellow-400' :
                    serverHealth.status === 'loading' ? 'text-gray-600' :
                      'text-red-700'
                }`
              }>
        ...
        >
          <div className='flex flex-col sm:flex-row items-center justify-start mb-4 text-emerald-800'>
            ...
            <div>
              ...
              <p className='text-xs md:text-sm font-light text-center sm:text-left'>don&apos;t worry, nothing is permanently stored. your chats are yours only.</p>
            </div>
          </div>
