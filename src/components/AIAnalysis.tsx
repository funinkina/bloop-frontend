  ...
  return (
    <div className={useSimpleStyles ? (profilesOnly ? "mb-5" : "mb-10") : (profilesOnly ? "mb-8" : "mb-16")}>
      ...
      {showSummary && (
        summary ? (
          <div>
            <p className={useSimpleStyles ? "text-3xl font-medium text-purple-900 mb-5" : "text-purple-900 text-2xl font-medium"}>
              {summary}
            </p>
          </div>
        ) : (
      ...
      {showProfiles ? (
              ...
              <div key={index} className={`${useSimpleStyles ? "bg-amber-50 p-4 rounded-lg border-2 border-gray-800" : "bg-amber-50 rounded-lg p-4 border-2 border-neutral-800"} group`}>
                <div className="flex items-center mb-2">
                  ...
                  <div className={useSimpleStyles ? "flex flex-col" : ""}>
                    <h4 className={useSimpleStyles ? "font-bold text-lg text-blue-950 w-full leading-none whitespace-nowrap" : "font-bold text-xl text-blue-950 whitespace-nowrap"}>
                      {person.name}
                    </h4>
                    <p className={useSimpleStyles ? "text-sm italic text-gray-800" : "text-sm text-gray-800 italic"}>
                      The {person.animal.charAt(0).toUpperCase() + person.animal.slice(1)} {people.length > 2 ? 'of the group' : ''}
                    </p>
                  </div>
                </div>
                <p className={useSimpleStyles ? "text-xl text-blue-950 font-medium" : "text-blue-950 text-lg font-medium"}>
                  {person.description}
                </p>
              </div>
            ))}
        ...
        profilesOnly && (
          <div className={useSimpleStyles ? "bg-gray-50 p-6 rounded-xl border-2 border-gray-800 shadow-[5px_5px_0px_0px_rgba(0,0,0,0.25)]" : "bg-gray-50 rounded-xl p-6 shadow-[5px_5px_0px_0px_rgba(0,0,0,0.25)] border-2 border-neutral-800"}>
            <p className={useSimpleStyles ? "text-gray-700 italic" : "text-gray-700 italic"}>
